...
<head>
    ...
    <script>
        function validateForm(event) {
            const nom = document.querySelector('[name="nom_utili"]').value.trim();
            ...
            const categorie = document.querySelector('[name="categorie_utili"]').value.trim();
            const email = document.querySelector('[name="email_utili"]').value.trim();
            const codeRegex = /^\d{5}$/;
            if (!nom || !mot_de_pass || !email || !categorie || isNaN(code) || !codeRegex.test(code)) {
...
<body>
    ...
    <form class="add" method="POST" onsubmit="validateForm(event)">
        ...
        <input type="text" id="nom_utili" name="nom_utili" required>
        
        <label for="mot_utili">Mot de passe</label>
        ...
        <label for="categorie_utili">Categorie</label>
        <input type="text" id="categorie_utili" name="categorie_utili" required>
